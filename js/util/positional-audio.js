...
export function playAudio(position) {
    if (!loaded || audioContext.state == 'running') {
        for (let source of audioSources) {
            source.position = [position[0], position[1], position[2]]
            source.source.setPosition(position[0], position[1], position[2])
            count += 0.05;
        }
        return;
    }
...
export function loadAudioSources(scene) {
    Promise.all([
        createAudioSource({
            url: '/media/sound/drum-hitnormal69.ogg',
